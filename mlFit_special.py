...
# Negative log-likelihood including measurement to constrain xi
def negLogL2(par):
    pdf = f(xData, par)
    xi = par[3]
...
# Initialize Minuit and set up fit, first with out aux. meas. u:
...
# Do the fit again with constraint on xi
m2 = Minuit(negLogL2, parin, name=parname)
m2.errors = parstep
m2.fixed = parfix
m2.limits = parlim
m2.errordef = 0.5      # errors from lnL = lnLmax - 0.5
m2.migrad()            # minimize -logL
MLE2 = m2.values                                    # max-likelihood estimates
sigmaMLE2 = m2.errors                               # standard deviations
cov2 = m2.covariance                                # covariance matrix
rho2 = m2.covariance.correlation()                  # correlation coeffs.
...
y_fitval = 0.8
delta_y_fitval = 0.08
plt.figtext(0.2, y_fitval, r'Maximum Likelihood with aux. measurement for $\xi$')
...
if not(m.fixed['theta'] | m.fixed['xi']):
    ...
    con2 = m2.mncontour('theta', 'xi', cl=CL, size=200)
    con2 = np.vstack([con2, con2[0]])         # close contour
    plt.plot(MLE[0], MLE[3], marker='o', linestyle='None', color='dodgerblue', label=r'$(\hat{\theta}, \hat{\xi})$')
    plt.plot(con[:,0], con[:,1], color='dodgerblue', linewidth=1, linestyle='dashed')
    plt.plot(MLE2[0], MLE2[3], marker='o', linestyle='None', color='black', label=r'with aux. meas.')
    plt.plot(con2[:,0], con2[:,1], color='black', linewidth=1)
...
if not(m.fixed['theta'] | m.fixed['xi']):
    ...
    m.draw_mncontour('theta', 'xi', cl=[0.683], size=200);
